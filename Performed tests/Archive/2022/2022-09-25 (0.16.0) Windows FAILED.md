...
Obsidian version: **0.16.3**
Shell commands version: **0.16.0**
Test date: **2022-09-25**
...
- [x] **New note folder name {{new_note_folder_name}} to file**: Should be *Performed tests*.
- [x] **Absolute new note folder path {{new_note_folder_path:absolute}} to file**
- [x] **Relative new note folder path {{new_note_folder_path:relative}} to file**
...
- [x] **Vault path {{vault_path}} to file**
- [x] **Workspace {{workspace}} to file**
- [x] **Test {{file content}}**: Open [[Welcome.md]] before executing this! Otherwise you'll get an error message saying that the shell command is too long. You should see the file's content **with YAML**.
- [x] **Test {{note content}}**: Open [[Welcome.md]] before executing this! Otherwise you'll get an error message saying that the shell command is too long. You should see the file's content **without YAML**.
...
- [ ] In the settings panel, pay attention to scrolling. SC's settings panel should remember the last scroll position when reopened. Tick this checkbox if it did remember. Exception: the scroll position is forgotten when Obsidian is restarted. ==Works kinda ok, except when there's a long command preview like `echo {{note_content}}` which takes some time to finish parsing, the height of the settings content will grow after the scroll happens, which means the scrolling is done too early. This can be fixed in some later release.==
...
MODAL OUTPUT
TEXT TO TOP
File created: 2022-09-25 (incomplete).md
Caret position: 28:27 (line: 28, column:27)
Clipboard: TESTCLIPBOARD
Date and time: 2022-09-25 10:02:39
File extension: with dot: .md / no dot: md
File name: 2022-09-25 (incomplete).mdMODAL OUTPUT

Absolute file path: V:\Lajitellut\Obsidian-tietokannat\Shell commands test\Performed tests\2022-09-25 (incomplete).md
Relative file path: Performed tests\2022-09-25 (incomplete).md
File URI: obsidian://vault/Shell%20commands%20test/Performed%20tests%2F2022-09-25%20(incomplete).md
...
Day of week: Sunday
Prompt mandatory field: dssddsds
Prompt field with a default value: Default
Prompt optional field: 
Prompt field using variables: 2022 
File created: TestResults.md~
File deleted: TestResults.md~
